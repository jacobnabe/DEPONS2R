...
#' @slot date POSIXlt object with the date and time when the simulation was
...
setClass(Class="DeponsTrack",
         slots=list(title="character", date="POSIXlt", crs="character",
         tracks="list")
)
...
setMethod("show", "DeponsTrack",
          function(object) {
            ...
            cat("date:\t\t", as.character(object@date), "\n")
            cat("crs:\t\t", object@crs, "\n")
...
get.sim.time <- function(fname=NULL) {
  ncf <- nchar(fname)
...
#' @param date Optional POSIXlt object with date of simulation. If
#' not provided this is obtained from name of input file
#' @param crs Character, coordinate reference system (map projection)
#'
#' @return Returns an object with the elements \code{title}, \code{date},
#' \code{crs}, and \code{tracks}. \code{date} is extracted from
#' the input data if not provided explicitly (a \code{POSIXlt} object. The
...
read.DeponsTrack <- function(fname, title="NA", date="NA", crs=CRS(as.character(NA))) {
  raw.data <- utils::read.csv(fname, sep=";")
  # Get sim date and time from file name
  if (date=="NA")  date <- get.sim.time(fname)
  tracks <- list()
